...
const Table = () => {
  ...
  detalTransaction?.map((trans, index) => {
    if (trans.vat === '23') {
      vat23.push(trans.netto * trans.ilosc)
    }
    if (trans.vat === '8') {
      vat8.push(trans.netto * trans.ilosc)
    }
    if (trans.vat === '0') {
    ...
    <div className='table'>
          ...
          {detalTransaction?.map((transaction, index) => {
            ...
            return (
              <tr key={index}>
                ...
                <td>
                  {(
                    (transaction.ilosc * transaction.netto * transaction.vat) /
                    100
                  ).toFixed(2)}
                </td>
                <td>
                  {(
                    transaction.ilosc * transaction.netto +
                    (transaction.ilosc * transaction.netto * transaction.vat) /
                      100
                  ).toFixed(2)}
                </td>
            ...
            <td colSpan='9'>
              ...
              <span>
                {detalTransaction
                  ?.reduce(
                    (acc, item) =>
                      acc + item.netto * item.ilosc * (1 + item.vat / 100),
                    0
                  )
                  .toFixed(2)}
              </span>
        ...
        <tbody>
          {vat23.length > 0 ? (
            <tr>
              ...
              <td>
                {vat23
                  .reduce((acc, item) => acc + (item * 23) / 100, 0)
                  .toFixed(2)}
              </td>
              <td>
                {vat23.reduce((acc, item) => acc + item * 1.23, 0).toFixed(2)}
              </td>
            </tr>
          ) : null}
          {vat8.length > 0 ? (
            <tr>
              ...
              <td>
                {vat8
                  .reduce((acc, item) => acc + (item * 8) / 100, 0)
                  .toFixed(2)}
              </td>
              <td>
                {vat8.reduce((acc, item) => acc + item * 1.08, 0).toFixed(2)}
              </td>
            </tr>
          ) : null}
          {vat0.length > 0 ? (
            <tr>
              ...
              <td>
                {vat0
                  .reduce((acc, item) => acc + (item * 0) / 100, 0)
                  .toFixed(2)}
              </td>
              <td>
                {vat0.reduce((acc, item) => acc + item * 1, 0).toFixed(2)}
              </td>
            </tr>
          ) : null}
          <tr>
            ...
            <td>
              {detalTransaction
                ?.reduce((acc, item) => acc + item.netto * item.ilosc, 0)
                .toFixed(2)}
            </td>
            <td>
              {detalTransaction
                ?.reduce(
                  (acc, item) =>
                    acc + (item.netto * item.ilosc * item.vat) / 100,
                  0
                )
                .toFixed(2)}
            </td>
      ...
      <div className='payment-wrapper'>
        <div className='method-payment'>
          Sposób zapłaty <span>{dataTransaction.way}</span>
        </div>
